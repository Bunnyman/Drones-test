    ...
    <div class="filter-bar" id="filterBar">
      ...
      <span class="time-group">
        ...
        <div class="win-seg" id="winSeg">
          ...
          <button data-win="3600">1 ГОД</button>
          <button data-win="7200" class="is-active">2 ГОД</button>
        </div>
...
// Достовірність: середня is OFF by default (only high-credibility events).
const enabledTags  = new Set(allTags.filter(t => t.toLowerCase() !== 'credibility: medium'));

function buildPillRow(host, items, enabledSet, labelFn, colorFn, extraClass, onChange) {
  host.innerHTML = '';
  for (const it of items) {
    const pill = document.createElement('span');
    pill.className = 'pill' + (enabledSet.has(it) ? ' active' : '')
